...
- `scripts/all-dataset-backbone-ddp.sh`: same wrapper but auto-detects system GPU count and launches each train.py via `torchrun --nproc_per_node=N` (DistributedSampler splits the batch, DDP syncs gradients each step)
