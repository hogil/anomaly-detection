...
| --- | ---: | ---: | ---: | ---: |
| 그냥 (no NT) | 5 | 0.9901 | 9.80 | 5.00 |
| NT=0.9 | 5 | 0.9920 | 6.80 | 5.20 |

NT 적용으로 FN 9.8 → 6.8 (-3.0). FP는 5.0 → 5.2 (+0.2)로 거의 변화 없음. F1 0.9901 → 0.9920 으로 향상. **순효과: 불량 더 잡고 정상은 거의 그대로** = 기본 NT=0.9 가 적절한 reporting threshold.

극단값(0.9999) 은 모든 케이스를 abnormal 로 분류해 FP=750(전체 normal) 이 되는 degenerate — test-peeking 위험 (memory `feedback_normal_threshold_099`).

재생성: `python scripts/generate_nt_effect_report.py --candidate-prefix fresh0412_v11_n700 --out-md validations/nt_effect.md --out-csv validations/nt_effect.csv --out-plot validations/nt_effect.png && cp validations/nt_effect.png docs/plots/`.
