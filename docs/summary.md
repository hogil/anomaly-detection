...
_자동 갱신 시각: `2026-04-29T12:24:41+09:00`._
...
- 서버 queue는 summary 축 기준으로 core 축을 먼저 보고, 후속 평가는 `color` -> `sample_skip` -> `logical_train` 순서로 분리한 뒤 `GC`를 마지막에 봅니다. GC는 5조건만 유지합니다.
...
| rawbase round1 live | rawbase history | 41 runs | 0.9981 | 0.634 | 2.195 | 중간 집계, 최종 claim 아님 |
...
| scope | 남은 내용 | runs |
| --- | --- | ---: |
| current rawbase queue | core axes 먼저 실행 후 `color`, `sample_skip`, `logical_train`, 마지막 `GC 5조건` | pending |
