...
| ch_08694 (std, scale=2.48) | ch_09036 (drift, 3.5σ) |
|---|---|
| ![](v9mid_journey/fn_examples/v4_fn_ch08694_std_scale2.48.png) | ![](v9mid_journey/fn_examples/v4_fn_ch09036_drift_3.5sigma.png) |
| 오른쪽 빨강 산포가 fleet 보다 "약간만" 넓음. 사람 눈으로도 애매. | 오른쪽 빨강이 살짝 아래로 경사. trend 인지 noise 인지 불분명. |

이 두 샘플이 **3 seeds 전부에서 반복 FN** (persistent hard case). defect 강도가 config range 하단이라 신호가 약함.

**해결**: range lower bound 올려서 이런 weak case 를 데이터셋에서 제거.
...
### 3.4 한계 확인 — v6 + n=2800 "이미 충분"

| 시도 | 변경 | test_f1 | 결론 |
|---|---|---|---|
| v6 | mean_shift boundary 추가 상승 | 99.89% (v5: 99.91%) | 변화 없음 — 새 경계에서 또 다른 hard case |
| n=2800 | normal 4배 증가 | 99.93% (n=700: 99.91%) | +0.02% 차이 — 동등 |

**결론**: v5 시점에서 **모델 capacity ceiling 도달**. boundary 를 더 올려도, 데이터를 더 줘도 의미 없음. n=700 에서 이미 모델이 학습 가능한 정보를 다 쓰고 있고, 더 주는 건 중복. 이 이후는 **학습 기법이나 이미지 품질 개선**으로만 성능 향상 가능.

### 3.5 Seed Robustness (v5, 6-seed 검증)

![Seed robustness](v9mid_journey/plots/06_seed_robustness.png)

v5 winning config 을 6 seeds (42/1/2/3/4/5) 로 검증. **F1 = 99.89 ± 0.03%** — seed 무관 안정.

---

이하 **기법 적용 단계** — v5 ceiling 돌파를 위한 학습/렌더링 개선.

---

## 3.6 Gradient Clipping Sweep
...
## 3.7 Target Color Experiment — Rendering 이 성능에 미치는 영향
...
## 3.8 Smoothed Median Val F1 — Best Model Selection 전략
