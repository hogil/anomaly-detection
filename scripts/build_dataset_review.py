...
이미지를 base64 로 박아 **파일 하나로 완결**되게 만든다 (외부 요청 0, 오프라인 열람 가능).
  ...
  python scripts/build_dataset_review.py --config configs/datasets/dataset_v12_n12k.yaml
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/datasets/dataset_v12_n12k.yaml")
