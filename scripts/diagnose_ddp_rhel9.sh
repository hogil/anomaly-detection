#!/usr/bin/env bash
# Diagnose CUDA/NCCL/DDP on RHEL9/HPC module environments.
#
# Usage:
#   MODULE_NAME=python/3.11.8 VENV_PATH=/path/to/venv bash scripts/diagnose_ddp_rhel9.sh
#   MODULE_NAME=python/3.11.8 VENV_PATH=/path/to/venv GPUS=0,1 bash scripts/diagnose_ddp_rhel9.sh
#   MODULE_NAME=none PYTHON=/path/to/python bash scripts/diagnose_ddp_rhel9.sh
set -uo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

MODULE_NAME="${MODULE_NAME:-python/3.11.8}"
...
section "RHEL9 module and venv setup"
...
if [[ "$MODULE_NAME" != "none" ]]; then
  if command -v module >/dev/null 2>&1; then
    ...
    echo "[warn] environment modules command not found; skipping module load $MODULE_NAME"
  fi
else
  echo "[info] MODULE_NAME=none; skipping module load"
fi
...
if [[ -n "$VENV_PATH" ]]; then
  if [[ -f "$VENV_PATH" ]]; then
    ...
    echo "[warn] VENV_PATH not found or not a venv: $VENV_PATH"
    status=1
  fi
fi
...
section "Summary"
if [[ "$status" -eq 0 ]]; then
  echo "PASS: CUDA/NCCL/DDP probes succeeded in this RHEL9/module environment."
else
