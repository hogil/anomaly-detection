...
batch 실행은 자동으로 `--log-dir-group run_<timestamp>`을 붙여서 모든 train.py 출력을 `logs/run_<timestamp>/<run>/` 아래에 모읍니다. 그룹별 표/plot은:
...
- 단일 학습은 `python train.py --log_dir my_run` — 그대로 `logs/my_run/`.
