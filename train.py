...
def main():
    ...
    parser.add_argument("--normal_threshold", type=float, default=0.7)
    ...
    parser.add_argument("--abnormal_weight", type=float, default=0.0,
                        help="Binary mode: abnormal class weight 배수 (0=자동)")
    ...
    parser.add_argument("--min_epochs", type=int, default=10,
                        help="최소 학습 epoch (이전에는 early stop 안 함)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch >= args.min_epochs and val_target_recall >= best_val_recall:
            is_strict_improvement = val_target_recall > best_val_recall
            best_val_recall = val_target_recall
            best_epoch = epoch
            if is_strict_improvement:
                patience_counter = 0  # 진짜 개선 → reset
            else:
                patience_counter += 1  # tie → stagnation 누적

            # 모델 저장 (latest tie weights)
            torch.save(model.state_dict(), str(log_dir / "best_model.pth"))
            tag = "NEW BEST" if is_strict_improvement else f"TIE update (stag {patience_counter}/{args.patience})"
            print(f"\n  * {tag} (target={val_target_recall:.4f}) -> model saved")
            ...
            nt_thresholds = [0.5, 0.6, 0.7, 0.8, 0.9]
            nt_results = {}
